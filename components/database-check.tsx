...
export function DatabaseCheck() {
  ...
  const checkTable = async () => {
    setLoading(true)
    setStatus('Checking if profiles table exists...')
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST116' || error.message.includes('does not exist')) {
          ...
          setStatus('❌ Profiles table does not exist')
        } else {
          setTableExists(false)
        ...
        setTableExists(true)
        setStatus('✅ Profiles table exists and is accessible')
      }
    } catch (err: any) {
      setTableExists(false)
    ...
    <Card className="w-full max-w-md">
      ...
      <CardContent className="space-y-4">
        ...
        {tableExists === false && (
          <Alert>
            <AlertDescription>
              The profiles table doesn't exist. Please run the SQL commands from the setup instructions to create it.
            </AlertDescription>
          </Alert>
        )}
