  ...
  if (code) {
    ...
    if (data.user) {
      ...
      try {
        ...
        // Check if user has client/buyer role (only clients allowed in client app)
        if (userRole !== 'client' && userRole !== 'buyer') {
          console.log('Access denied - user role is:', userRole)
          // Redirect to role-access page with user information
          redirect('/role-access')
        }

        // Redirect to the main page for authorized users (clients/buyers)
        redirect("/")
      } catch (err) {
